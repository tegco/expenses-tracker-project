...
exports.getIncomeById = async (req, res) => {
  ...
  try {
    const user_income = await db.query("SELECT * FROM income WHERE id = $1", [
      incomeId,
    ]);
  ...
  try {
    ...
    if (userIncome && userIncome.length > 0) {
      ...
      res
        ...
        .json({
          message:
            "No income found for the user in the specified month and year.",
        });
    }
  } catch (error) {
    console.error("Error fetching income:", error);
...
exports.deleteIncome = async (req, res) => {
  try {
    ...
    const incomeDate = await db.query(
      "SELECT income_date FROM income WHERE id = $1",
      [incomeId]
    );
    const result = await db.query(
    ...
    if (result.length === 0) {
      ...
      const [year, month, day] = incomeDate[0].income_date
        .toISOString()
        .split("T")[0]
        .split("-");
      console.log(
        "IncomeDate split ->",
        incomeDate[0].income_date.toISOString().split("T")[0].split("-")
      );
