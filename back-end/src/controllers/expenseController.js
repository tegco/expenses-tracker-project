...
exports.createExpense = async (req, res) => {
  ...
  const { expense_date, description, amount, payment_method_id, category_id } =
    req.body;
  const loggedUserId = req.user.id;
  const result = await db.query(
    "SELECT selected_currency_id FROM app_user WHERE id = $1",
    [loggedUserId]
  );
...
exports.getExpensesByMonthAndYear = async (req, res) => {
  try {
    ...
    const loggedUserId = req.user.id;
    const result = await db.query(
      "SELECT * FROM expense WHERE user_id = $1 AND EXTRACT(YEAR FROM expense_date) = $2 AND EXTRACT(MONTH FROM expense_date) = $3",
...
exports.getExpenseById = async (req, res) => {
  try {
    ...
    const expense = await db.query("SELECT * FROM expense WHERE id = $1", [
      expenseId,
    ]);
...
exports.deleteExpense = async (req, res) => {
  try {
    ...
    const expenseDate = await db.query(
      "SELECT expense_date FROM expense WHERE id = $1",
      [expenseId]
    );
    const result = await db.query(
    ...
    if (result.length === 0) {
      ...
      const [year, month, day] = expenseDate[0].expense_date
        .toISOString()
        .split("T")[0]
        .split("-");
...
exports.getExpensesByCategoryId = async (req, res) => {
  try {
    ...
    const expenses = await db.query(
      "SELECT * FROM expense WHERE user_id = $1 AND category_id = $2",
