...
const updateMonthlyTotals = async (userId, year, month, type) => {
  try {
    ...
    const existingTotal = currentMonthlyTotal[`${year}-${month}`] || {
      total_expense: 0,
      total_income: 0,
    };
    ...
    const result = await db.query(
      `SELECT COALESCE(SUM(amount)) AS total_${type}
         ...
         WHERE user_id = $1 AND EXTRACT(YEAR FROM ${
           type === "expense" ? "expense" : "income"
         }_date) = $2 AND EXTRACT(MONTH FROM ${
        type === "expense" ? "expense" : "income"
      }_date) = $3`,
      [userId, year, month]
    );
    ...
    await db.query("UPDATE app_user SET monthly_totals = $1 WHERE id = $2", [
      updatedMonthlyTotal,
      userId,
    ]);
  } catch (error) {
...
const getMonthlyTotals = async (userId) => {
  const result = await db.query(
    "SELECT monthly_totals FROM app_user WHERE id = $1",
    [userId]
  );
  return result || {};
};
